...
export const DocumentController = {
   ...
   * POST /api/documents/telegram/generate
   */
  async generateDocumentTelegram(req: Request, res: Response): Promise<void> {
    try {
      const { situation, document_id } = req.body;

      if (!situation || typeof situation !== 'string' || !document_id) {
        res.status(400).json({
          success: false,
      ...
      const template = await DocumentService.prepareDocument(
        situation,
        document_id,
      );
      ...
      const general_prompt = PromptService.getMainPrompt();
      const prompt = PromptService.getFirstTemplatePrompt(
        general_prompt,
        template,
      );
