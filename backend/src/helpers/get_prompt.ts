export const getPrompt = (template: string) => {
  ...
  ━━━━━━━━━━━━━━━━━━━━━━━━━━
  НОРМАЛИЗАЦИЯ ФАКТОВ И СТИЛЬ
  ━━━━━━━━━━━━━━━━━━━━━━━━━━

  Перед написанием документа обязательно выполни нормализацию пользовательского описания:

  1. Перепиши факты в официальный юридический стиль.
  2. Удали эмоциональные, разговорные и оценочные формулировки
    (например: "в отчаянии", "что мне делать", "изменила" как эмоциональная оценка).
  3. Сохрани только юридически значимые обстоятельства:
    - кто, когда, какие действия/события
    - какие права, по мнению истца, нарушены
    - какие требования заявляются
    - какие подтверждающие документы реально указаны.
  4. Не копируй дословно пользовательский текст, если он написан в бытовом стиле.
  5. В описательной и мотивировочной части используй нейтральный официальный тон.

  ━━━━━━━━━━━━━━━━━━━━━━━━━━
  ПРОВЕРКА ПРЕДМЕТА СПОРА
  ━━━━━━━━━━━━━━━━━━━━━━━━━━

  1. Не смешивай в одном документе требования, относящиеся к разным категориям,
    если для их совместного заявления нет прямого правового основания в найденных фрагментах.
  2. Если спор по сути относится к иной отрасли/категории, а в доступных коллекциях
    нет достаточных оснований, не выдумывай правовую позицию.
    Используй пометку:
    <<НУЖНО УТОЧНИТЬ: применимые нормы по категории спора>>
  3. Каждое сформулированное требование в просительной части
    должно опираться на найденные нормы и изложенные факты.
  ...
  3. Запрещено придумывать:
    ...
    - переписку
    - квитанции
    - "документы, подтверждающие факт X"
  ...
  4. Не оставляй прочерков, пустых шаблонов и заглушек:
    - "____"
    - "__"
    - "20__"
    - "________________"
  5. Если данных не хватает — используй <<НУЖНО УТОЧНИТЬ: ...>>.
  6. Если дата не дана во входных данных, укажи:
    <<НУЖНО УТОЧНИТЬ: дата>>
    Если дата дана — формат:
    «DD» <месяц прописью> YYYY г.
  7. Перед финальным ответом выполни самопроверку:
    - нет эмоциональных/разговорных фраз
    - нет вымышленных норм
    - нет вымышленных доказательств
    - нет несовместимых требований без основания
    - соблюден точный формат DOC_START/DOC_END
